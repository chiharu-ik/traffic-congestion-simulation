...
st.write("本シミュレーションでは、曜日・時間帯から交通量を決定し、交通量を最大処理交通量で割ることで需要率を求めています。")
st.write("需要率をWebsterの近似式に代入し、信号サイクル長Cと青信号時間を求めています。")
st.write("前方車両数と車種ごとの台数は手動で設定し、最後尾車両の通過時間Tを計算します。")
st.write("Tが青信号時間を超えた場合、1回の青信号で処理しきれないため渋滞と判定します。")
